...
ASK_MODEL = os.getenv("OPENAI_MODEL", "gpt-4.1-mini")
ASK_TEMPERATURE = float(os.getenv("ASK_TEMPERATURE", "0.35"))
ASK_MAX_TOKENS = int(os.getenv("ASK_MAX_TOKENS", "450"))

STOCKSENSEI_SYSTEM_PROMPT = """
You are StockSensei's educational assistant for a student-built stock analysis app.

Allowed:
- Explain stock market terms, app metrics, model outputs, charts, confidence, volatility, and watchlist concepts.
- Explain how StockSensei's predictions should be interpreted for learning.
- Give beginner-friendly research checklists and questions a user can ask before making decisions.
- Use clear, concise language for high school or early college students.

Not allowed:
- Do not give personalized financial advice.
- Do not tell the user to buy, sell, hold, short, or trade a specific security.
- Do not promise returns or claim StockSensei predictions are guaranteed.
- Do not ask for sensitive personal financial information.
- Do not provide legal, tax, or professional investment advice.

Response format:
1. Start with a direct 1-2 sentence answer.
2. Add 2-4 short bullet points if helpful.
3. End with: "Educational only, not financial advice."
"""


def get_openai_client():
    if OpenAI is None:
        return None, "OpenAI package is not installed. Run: python3 -m pip install -r requirements.txt"

    if not os.getenv("OPENAI_API_KEY"):
        return None, "OPENAI_API_KEY is missing. Add it to backend/.env or your deployment environment variables."

    return OpenAI(api_key=os.getenv("OPENAI_API_KEY")), None


def answer_learn_question(question):
    ...
    if len(question) > 900:
        return None, "Please keep your question under 900 characters.", 400

    client, setup_error = get_openai_client()
    if setup_error:
        return None, setup_error, 503
    ...
    try:
        response = client.responses.create(
            ...
            instructions=STOCKSENSEI_SYSTEM_PROMPT,
            input=question
        )
        ...
        if not answer:
            answer = "I could not generate an answer right now. Try again with a shorter question."
        ...
        return None, f"LLM request failed: {error}", 500
...
def ask_question():
    ...
    data, error, status = answer_learn_question(question)
